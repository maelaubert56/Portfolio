...
function ChangeLanguage() {
  ...
  return (
    <div className="ChangeLanguage">
      <button id="dropdown-btn" onClick={() => setDropdown(!dropdown)}>
        {t("Header_Links_Language")}
      </button>
      <div id="dropdown-content" className={dropdown ? "show" : ""}></div>
    </div>
  );
}
...
function Header({ theme, setTheme }) {
  ...
  return (
    <header>
      <img
        src={theme === "light" ? "/logo250.png" : "/icon_white.svg"}
        alt="Logo"
        id="Logo"
      />
      <ul>
